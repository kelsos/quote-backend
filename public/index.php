...
$app->post('/oauth', function (Request $request, Response $response, $args = []) {
    ...
    $client = new Google_Client(['client_id' => $stateManager->getOauthClientId()]);
    $payload = $client->verifyIdToken($body->{'oauth_token'});
    if ($payload) {
        $userid = $payload['sub'];
        ...
        if ($user == null) {
            $created = $userManager->createOAuthUser($userid, $username);
            $result = [
                "success" => $created,
                "code" => Constants::SUCCESS
            ];
            ...
            $userToken = $userManager->createUserToken($user);
