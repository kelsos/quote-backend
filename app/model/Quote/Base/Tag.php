...
abstract class Tag implements ActiveRecordInterface
{
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildQuoteTag[]
     */
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->quoteTagsScheduledForDeletion !== null) {
                if (!$this->quoteTagsScheduledForDeletion->isEmpty()) {
                    \Quote\QuoteTagQuery::create()
                        ->filterByPrimaryKeys($this->quoteTagsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->quoteTagsScheduledForDeletion = null;
                }
            }

            if ($this->collQuoteTags !== null) {
                foreach ($this->collQuoteTags as $referrerFK) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collQuoteTags) {
                ...
                $result[$key] = $this->collQuoteTags->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getQuoteTags() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addQuoteTag($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('QuoteTag' == $relationName) {
            return $this->initQuoteTags();
        }
    }

    /**
     * Clears out the collQuoteTags collection
     *
     ...
     * @see        addQuoteTags()
     */
    public function clearQuoteTags()
    {
        $this->collQuoteTags = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collQuoteTags collection loaded partially.
     */
    public function resetPartialQuoteTags($v = true)
    {
        $this->collQuoteTagsPartial = $v;
    }

    /**
     * Initializes the collQuoteTags collection.
     *
     * By default this just sets the collQuoteTags collection to an empty array (like clearcollQuoteTags());
     ...
     * @return void
     */
    public function initQuoteTags($overrideExisting = true)
    {
    ...
    /**
     * Gets an array of ChildQuoteTag objects which contain a foreign key that references this object.
     *
     ...
     * @throws PropelException
     */
    public function getQuoteTags(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collQuoteTagsPartial && !$this->isNew();
        if (null === $this->collQuoteTags || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collQuoteTags) {
                ...
                if (null !== $criteria) {
                    if (false !== $this->collQuoteTagsPartial && count($collQuoteTags)) {
                        $this->initQuoteTags(false);

                        foreach ($collQuoteTags as $obj) {
                            if (false == $this->collQuoteTags->contains($obj)) {
                                $this->collQuoteTags->append($obj);
                            }
                        }

                        $this->collQuoteTagsPartial = true;
                    }

                    return $collQuoteTags;
                }

                if ($partial && $this->collQuoteTags) {
                    foreach ($this->collQuoteTags as $obj) {
                        if ($obj->isNew()) {
                            $collQuoteTags[] = $obj;
                        }
                    }
                ...
                $this->collQuoteTags = $collQuoteTags;
                $this->collQuoteTagsPartial = false;
            }
        }

        return $this->collQuoteTags;
    }

    /**
     * Sets a collection of ChildQuoteTag objects related by a one-to-many relationship
    ...
    {
        /** @var ChildQuoteTag[] $quoteTagsToDelete */
        $quoteTagsToDelete = $this->getQuoteTags(new Criteria(), $con)->diff($quoteTags);
    ...
    /**
     * Returns the number of related QuoteTag objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related QuoteTag objects.
     * @throws PropelException
     */
    public function countQuoteTags(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collQuoteTagsPartial && !$this->isNew();
        if (null === $this->collQuoteTags || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collQuoteTags) {
            ...
            if ($partial && !$criteria) {
                return count($this->getQuoteTags());
            }
        ...
        return count($this->collQuoteTags);
    }

    /**
     * Method called to associate a ChildQuoteTag object to this object
    ...
    /**
     * @param ChildQuoteTag $quoteTag The ChildQuoteTag object to add.
     */
    ...
    {
        if ($this->getQuoteTags()->contains($quoteTag)) {
            $pos = $this->collQuoteTags->search($quoteTag);
            $this->collQuoteTags->remove($pos);
            if (null === $this->quoteTagsScheduledForDeletion) {
                $this->quoteTagsScheduledForDeletion = clone $this->collQuoteTags;
    ...
    {
        if ($deep) {
            if ($this->collQuoteTags) {
                foreach ($this->collQuoteTags as $o) {
